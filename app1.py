...
def display(message, qid):
    ...
    for i in range(1):
        with col_a:
            render_latex_textblock(message["gen_question_spark"])
        with col_b:
            render_latex_textblock(message["gen_question_glm"])
        with col_c:
            render_latex_textblock(message["gen_question_o4"])
...
def render_scoring(qid: str):
    ...
    models = ["模型spark", "模型glm", "模型o4"]
    model_keys = ["spark", "glm", "o4"]
    ...
    for dim, control_type in dimensions.items():
        ...
        for i, model in enumerate(models):
            ...
            key_score = f"{key}_{dim}_score_{model_key}"
            key_comment = f"{key}_{dim}_comment_{model_key}"

            scores[key].setdefault(model_key, {})

            prev_score = scores[key][model_key].get(f"{cleaned_dim}_scores", "")
            prev_comment = scores[key][model_key].get(f"{cleaned_dim}_comments", "")
            ...
            if control_type == "radio":
                val_score = cols[i].radio(model, options, key=key_score, index=index)
                ...
                if val_score is not None:
                    scores[key][model_key][f"{cleaned_dim}_scores"] = val_score
                else:
                    scores[key][model_key][f"{cleaned_dim}_scores"] = ""

                scores[key][model_key][f"{cleaned_dim}_comments"] = comment
